...
function processedCount(summary: Awaited<ReturnType<typeof runOrderMaintenanceJobs>>) {
  return summary.checkoutExpiry.expired
    + summary.confirmationTimeout.cancelled
    + summary.deliveryTimeout.completed
}
...
const workerName = 'order_maintenance'
...
function requestStop(signal: string) {
  stopping = true
  console.log(JSON.stringify({ ok: true, event: 'order_maintenance_worker_stop_requested', signal }))
}
...
async function main() {
  ...
  console.log(JSON.stringify({
    ok: true,
    event: 'order_maintenance_worker_started',
    limit,
    confirmationOlderThanHours,
    deliveryOlderThanHours,
    intervalMs,
  ...
  while (!stopping) {
    ...
    const processed = processedCount(summary)
    ...
    console.log(JSON.stringify({ ok: true, event: 'order_maintenance_worker_run', run, processed, ...summary }))
  ...
  console.log(JSON.stringify({ ok: true, event: 'order_maintenance_worker_stopped', runs: run, idleRuns }))
}
