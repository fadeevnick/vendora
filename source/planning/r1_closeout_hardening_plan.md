...
H1 Stripe payment provider adapter code now exists behind PAYMENT_PROVIDER=stripe. It creates Stripe Checkout Session requests, preserves raw webhook body for signature verification and maps signed checkout.session.completed events to PAYMENT_SUCCEEDED. npm run runtime:h1-stripe-payment-provider succeeded with H1-STRIPE-PAYMENT-PROVIDER-01 through H1-STRIPE-PAYMENT-PROVIDER-03 on 2026-05-10. This is adapter-code/signature-contract proof, not live Stripe dashboard/API proof.
H1 Stripe refund provider adapter code now exists behind REFUND_PROVIDER=stripe. Payment webhook handling retains provider payment intent evidence, and Stripe refunds require that evidence before creating provider refund requests. npm run runtime:h1-stripe-refund-provider succeeded with H1-STRIPE-REFUND-PROVIDER-01 and H1-STRIPE-REFUND-PROVIDER-02 on 2026-05-10. This is adapter-code/refund-contract proof, not live Stripe refund dashboard/API proof.
